...
class EasyBarberSupabaseUser extends BaseAuthUser {
  EasyBarberSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> easyBarberSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EasyBarberSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
